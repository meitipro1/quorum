...
This script does the same three things the reviewer does:
    ...
    2. compares it with contracts/quorum.py, identical up to line endings and
       the final newline, and reports a cosmetic difference separately from a
       material one
    3. runs `genvm-lint lint` on the bytes that came off the chain

It exits non-zero if any of them fails, so it can gate a submission. It fails
closed: without genvm-lint installed (pip install genvm-linter) it says so and
refuses, rather than reporting a pass it never checked.
"""
...
SOURCE = ROOT / "contracts" / "quorum.py"
...
def lint(text):
    ...
    try:
        ...
        # Fail closed. A gate that cannot run its check must not report a pass.
        return False, "genvm-lint is not installed: pip install genvm-linter"
